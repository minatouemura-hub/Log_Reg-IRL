...
def main():
    ...
    for target in ["F", "M"]:
        ...
        user_list = {"user_id": [], "s_score": [], "d_score": [], "gr": []}
        for expert_id in tqdm(expert_numbers, desc="experts_train", leave=True):
            ...
            for flag in ["M", "F"]:
                ...
                if flag == target:
                    user_list["s_score"].append(sum_score)
                    two_score_dict["S_expert_score"].append(sum_score)
                    S_score = sum_score
                else:
                    user_list["d_score"].append(sum_score)
        ...
        accuracy = plot_GenderRate(num, two_score_dict=user_list, target_gender=target)
        print("判別精度：", accuracy)
